-- Older quotations can predate the required sales_profile_id. Revisions must
-- remain possible without bypassing the invariant that a new quotation always
-- belongs to an active SALE profile.
...
begin
  ...
  -- Prefer the salesperson recorded on the source quotation. Legacy documents
  -- may not have one, or may point to a deactivated account, so fall back to
  -- the current SALE user and finally to the first active SALE profile.
  select p.* into v_sales
  from public.profiles p
  where p.id = v_source.sales_profile_id
    and p.role = 'SALE'
  ...
  if not found then
    raise exception 'No active salesperson is available to create a revision';
  end if;
