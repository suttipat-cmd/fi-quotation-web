-- Structured fields used by the two-page quotation format.
alter table public.quotations
  add column if not exists package_reference_quantity numeric(12,2),
  add column if not exists package_reference_unit text,
  add column if not exists included_users integer check (included_users is null or included_users >= 0),
  add column if not exists billing_cycle text,
  add column if not exists additional_fees text,
  add column if not exists promotion_terms text;

-- Preserve the new document content when creating an editable revision.
...
begin
  ...
  insert into public.quotations (
    ...
    v_source.additional_fees, v_source.promotion_terms, (select auth.uid()), (select auth.uid())
