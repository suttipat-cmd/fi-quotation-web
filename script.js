...
// Scope:
// - Move Drive / Send actions from #quotation-print to #quotation-view.
// - Keep #quotation-print focused on preview and browser PDF only.
// - Show product/service names from current Product Master via product_id, with one-time/manual rows as fallback.
// - Add product filter and product column on quotation list.
// - Fix funnel denominator, improve pie colors, and render paid trend as vertical bar chart.
// =======================================================

const FI_V1121_VERSION = "1.12.1";
window.FI_APP_VERSION = FI_V1121_VERSION;

if (!appState.quotationFilters) appState.quotationFilters = {};
if (!Object.prototype.hasOwnProperty.call(appState.quotationFilters, "productId")) {
  appState.quotationFilters.productId = "";
}

function uniqueValuesV1121(values) {
  return Array.from(new Set((values || []).filter(Boolean)));
}

function getProductDisplayNameV1121(product) {
  if (!product) return "";
  const code = String(product.code || "").trim();
  const name = String(product.name || "").trim();
  return [code, name].filter(Boolean).join(" ") || name || code || "สินค้า/บริการ";
}

function getItemDisplayNameV1121(item = {}) {
  if (item.product_id && item.product_master_name_v1121) return item.product_master_name_v1121;
  if (item.product_id && item.product_name) return item.product_name;
  return item.product_name_snapshot || item.name || "สินค้า/บริการ";
}

function getPrimaryProductLabelV1121(row = {}) {
  return row.product_name_v1121 || row.product_name || row.product_name_snapshot || row.primary_product_name || "-";
}

async function loadProductsMapV1121(productIds = null) {
  let query = supabaseClient
    .from("products")
    .select("id, code, name, default_unit, is_active")
    .order("created_at", { ascending: true });

  const ids = uniqueValuesV1121(productIds || []);
...
async function loadActiveProductsV1121() {
  const { data, error } = await supabaseClient
    .from("products")
    .select("id, code, name, default_unit, is_active")
    .eq("is_active", true)
...
function enrichQuotationItemsWithProductMasterV1121(items = [], productsMap = new Map()) {
  return (items || []).map((item) => {
    const product = item.product_id ? productsMap.get(item.product_id) : null;
    const masterName = product ? getProductDisplayNameV1121(product) : "";
    return {
      ...item,
      product_master_v1121: product || null,
      product_master_name_v1121: masterName,
      product_name_snapshot: masterName || item.product_name_snapshot,
    };
  });
}

async function loadQuotationItemsForRowsV1121(quotationIds = []) {
  const ids = uniqueValuesV1121(quotationIds || []);
  if (!ids.length) return { items: [], productsMap: new Map() };

  const { data, error } = await supabaseClient
    .from("quotation_items")
    .select("quotation_id, section_type, product_id, product_name_snapshot, sort_order, line_subtotal, quantity, unit_price")
    .in("quotation_id", ids)
    .order("section_type", { ascending: false })
    .order("sort_order", { ascending: true });
  ...
  const items = data || [];
  const productsMap = await loadProductsMapV1121(items.map((item) => item.product_id));
  return { items: enrichQuotationItemsWithProductMasterV1121(items, productsMap), productsMap };
}

function selectPrimaryQuotationItemV1121(items = []) {
  return (items || []).find((item) => item.section_type === "recurring" && item.product_id) ||
    (items || []).find((item) => item.section_type === "recurring") ||
    (items || [])[0] || null;
}

async function enrichQuotationRowsWithPrimaryProductV1121(rows = []) {
  const ids = rows.map((row) => row.id).filter(Boolean);
  const { items } = await loadQuotationItemsForRowsV1121(ids);
  const itemsByQuotation = new Map();
  items.forEach((item) => {
    if (!itemsByQuotation.has(item.quotation_id)) itemsByQuotation.set(item.quotation_id, []);
    itemsByQuotation.get(item.quotation_id).push(item);
  });

  return rows.map((row) => {
    const primary = selectPrimaryQuotationItemV1121(itemsByQuotation.get(row.id) || []);
    return {
      ...row,
      product_id_v1121: primary?.product_id || "",
      product_name_v1121: primary ? getItemDisplayNameV1121(primary) : "-",
    };
  });
}

async function loadQuotationPrintModelV1121(quotationId) {
  const [quotationResult, itemsResult, defaultCompanyResult, ownerResult, sectionTotalsResult] = await Promise.all([
    supabaseClient.from("quotations").select("*").eq("id", quotationId).single(),
    supabaseClient
      .from("quotation_items")
    ...
    supabaseClient
      ...
      .then(async (quotationOwnerResult) => {
        ...
        const ownerId = quotationOwnerResult.data?.owner_id;
        if (!ownerId) return { data: null, error: null };
        const profileResult = await supabaseClient.from("profiles").select("full_name, email, phone").eq("id", ownerId).maybeSingle();
        return {
          data: {
            sales_name_snapshot: quotationOwnerResult.data?.sales_name_snapshot,
            profile: profileResult.data,
  ...
  const quotation = quotationResult.data;
  const productsMap = await loadProductsMapV1121((itemsResult.data || []).map((item) => item.product_id));
  const items = enrichQuotationItemsWithProductMasterV1121(itemsResult.data || [], productsMap);
  const defaultCompany = defaultCompanyResult.data || {};
  const ownerName =
    ownerResult.data?.profile?.full_name ||
    ownerResult.data?.profile?.email ||
  ...
  if (!sectionTotalsResult.error && Array.isArray(sectionTotalsResult.data)) {
    ...
    console.warn("calculate_quotation_section_totals fallback v1.12.1", sectionTotalsResult.error);
    sectionTotals = calculateSectionTotalsLocal(quotation, items);
  }
  ...
  model.ownerProfileV1121 = ownerResult.data?.profile || null;
  model.itemsV1121 = items;
  return model;
}

function createHiddenPrintHostForDriveV1121(model) {
  const host = document.createElement("div");
  host.id = "hiddenPrintHostV1121";
  host.style.cssText = "position:fixed;left:-10000px;top:0;width:210mm;opacity:0;pointer-events:none;z-index:-1;";
  host.innerHTML = renderPrintV15(model);
  document.body.appendChild(host);
  return host;
}

const originalHandleSaveQuotationPdfToDriveV1121 = typeof handleSaveQuotationPdfToDriveV1102 === "function" ? handleSaveQuotationPdfToDriveV1102 : null;
if (originalHandleSaveQuotationPdfToDriveV1121) {
  handleSaveQuotationPdfToDriveV1102 = async function handleSaveQuotationPdfToDriveV1121(model) {
    let hiddenHost = null;
    try {
      if (!document.getElementById("quotationPrintPage") && !document.querySelector(".print-v2-page") && model?.quotation?.id) {
        const richModel = model.sections ? model : await loadQuotationPrintModelV1121(model.quotation.id);
        hiddenHost = createHiddenPrintHostForDriveV1121(richModel);
        await new Promise((resolve) => requestAnimationFrame(resolve));
        return await originalHandleSaveQuotationPdfToDriveV1121(richModel);
      }
      return await originalHandleSaveQuotationPdfToDriveV1121(model);
    } finally {
      hiddenHost?.remove();
      if (model?.quotation?.id && getPageFromHash?.() === `quotation-view/${model.quotation.id}`) {
        window.setTimeout(() => renderQuotationViewPage(model.quotation.id).catch(console.error), 250);
      }
    }
  };
}

function canManageQuotationV1121(quotation = {}) {
  const role = appState.profile?.role;
  const isOwner = quotation.owner_id === appState.user?.id;
  return role === "admin" || (role === "sales" && isOwner);
}
...
function renderQuotationActionButtons(quotation, effectiveStatus) {
  if (!canManageQuotationV1121(quotation)) return "";
  const buttons = [];
  const status = quotation.status;
  ...
  if (status !== "draft") {
    buttons.push(`<button id="printPreviewButton" class="btn btn-ghost">ตัวอย่างเอกสาร / พิมพ์</button>`);
    buttons.push(`<button id="duplicateQuotationButton" class="btn btn-ghost">สร้างสำเนา</button>`);
  }
...
function renderQuotationItemsTableV1121(items) {
  if (!items.length) return `<div class="empty-state">ไม่มีรายการในส่วนนี้</div>`;
  return `
    <div class="table-wrap">
      <table class="data-table">
        <thead>
          ...
          ${items.map((item, index) => {
            ...
            return `
              <tr>
                <td>${index + 1}</td>
                <td>
                  <strong>${escapeHTML(getItemDisplayNameV1121(item))}</strong>
                  ${item.description ? `<div class="inline-note">${escapeHTML(item.description)}</div>` : ""}
                </td>
                <td>${number(item.quantity)}</td>
                <td>${escapeHTML(item.unit || item.product_master_v1121?.default_unit || "-")}</td>
                <td>${formatTHB(item.unit_price)}</td>
                <td>${formatTHB(lineSubtotal)}</td>
              </tr>
            `;
...
function renderDeliverySummaryV1121(quotation = {}) {
  const hasDelivery = Boolean(
    quotation.sent_at || quotation.delivery_planned_sent_at ||
    quotation.sent_recipient_email || quotation.delivery_recipient_email ||
    quotation.sent_recipient_name || quotation.delivery_recipient_name
  );
  if (!hasDelivery) return `<div class="empty-state compact">ยังไม่มีข้อมูลการส่งใบเสนอราคา</div>`;

  const status = quotation.status || quotation.effective_status || "";
  const finalized = status === "sent" || status === "paid" || Boolean(quotation.sent_email_sent_at || quotation.sent_at);
  const subtitle = status === "paid"
    ? "ส่งอีเมลแล้ว และบันทึกสถานะชำระเงินแล้ว"
    : finalized
      ? "ส่งอีเมลและเปลี่ยนสถานะเป็นส่งแล้ว"
      : "บันทึกข้อมูลผู้รับแล้ว แต่ยังไม่ได้ส่งอีเมล";

  return `
    <div class="sent-summary-v1111 sent-summary-v1113 sent-summary-v1115 ${finalized ? "is-sent" : "is-draft"} ${status === "paid" ? "is-paid" : ""}">
      <div class="sent-summary-head-v1111">
        <span class="sent-summary-icon-v1111">${finalized ? "✓" : "•"}</span>
        <div><strong>ข้อมูลการส่งใบเสนอราคา</strong><small>${escapeHTML(subtitle)}</small></div>
      </div>
      <div class="sent-summary-grid-v1111">
        <div><span>วันที่ส่ง</span><b>${formatDate(quotation.sent_at || quotation.delivery_planned_sent_at)}</b></div>
        <div><span>อีเมลผู้รับ</span><b>${escapeHTML(quotation.sent_recipient_email || quotation.delivery_recipient_email || "-")}</b></div>
        <div><span>ผู้รับ</span><b>${escapeHTML(quotation.sent_recipient_name || quotation.delivery_recipient_name || "-")}</b></div>
        <div><span>ตำแหน่ง</span><b>${escapeHTML(quotation.sent_recipient_position || quotation.delivery_recipient_position || "-")}</b></div>
        ${finalized ? `<div><span>ส่งอีเมลเมื่อ</span><b>${quotation.sent_email_sent_at ? formatDate(quotation.sent_email_sent_at) : "-"}</b></div>` : ""}
      </div>
    </div>
  `;
}

function renderQuotationDocumentManagerV1121(model, driveLog, settings) {
  const quotation = model.quotation;
  if (quotation.status === "draft") return "";
  return `
    <section class="card document-manager-v1121">
      <div class="card-header compact-v1120">
        <div>
          <h3>จัดการเอกสารและการส่ง</h3>
          <p>บันทึก PDF ลง Google Drive และบันทึกข้อมูลการส่งจากหน้านี้</p>
        </div>
      </div>
      <div id="driveArchiveStatusV1102" class="drive-archive-status-v1102 view-drive-status-v1121">
        <span class="drive-status-text-v1102">กำลังตรวจสถานะ Google Drive...</span>
      </div>
      <div class="delivery-summary-panel-v1121">
        ${renderDeliverySummaryV1121(quotation)}
      </div>
    </section>
  `;
}

async function renderQuotationViewPageV1121(quotationId) {
  if (!quotationId) {
    renderError("ไม่พบรหัสใบเสนอราคา");
    return;
  }

  setCompactPageHeaderV1120("รายละเอียดใบเสนอราคา", "ตรวจสอบและจัดการเอกสาร");
  renderLoading("กำลังโหลดใบเสนอราคา...");

  const model = await loadQuotationPrintModelV1121(quotationId);
  const quotation = model.quotation;
  const items = model.itemsV1121 || model.sections.flatMap((section) => section.items || []);
  const recurringItems = items.filter((item) => item.section_type === "recurring");
  const oneTimeItems = items.filter((item) => item.section_type === "one_time");
  const effectiveStatus = getEffectiveStatusFromQuotation(quotation);
  ...
  if (quotation.status !== "draft") {
    try {
      [settings, driveLog] = await Promise.all([
        loadDriveArchiveSettingsV1102(),
        loadDriveFileLogV1102(quotation.id),
      ]);
      appState.driveArchiveSettingsV1102 = settings;
    } catch (error) {
      console.warn("Cannot load Drive status in view v1.12.1", error);
      driveLog = { loadError: error.message || String(error) };
    }
  }
  ...
  elements.pageContent.innerHTML = `
    <div class="card">
      <div class="card-header">
        ...
        <div class="table-actions view-actions-v1121">
          <button id="backToListButton" class="btn btn-ghost">กลับไปหน้ารายการ</button>
          ${renderQuotationActionButtons(quotation, effectiveStatus)}
        </div>
      </div>
      ...
      <div class="alert alert-warning ${quotation.status === "draft" ? "" : "hidden"}">
        เอกสารนี้ยังเป็นร่าง ยืนยันเอกสารเพื่อสร้างเลขใบเสนอราคา
      </div>

      <div class="kv-list">
        <div class="kv-row"><span>สถานะ</span><strong>${statusBadge(effectiveStatus)}</strong></div>
        <div class="kv-row"><span>ลูกค้า</span><strong>${escapeHTML(quotation.customer_name || "-")}</strong></div>
        <div class="kv-row"><span>ที่อยู่ลูกค้า</span><strong>${escapeHTML(quotation.customer_address || "-")}</strong></div>
        <div class="kv-row"><span>วันที่เสนอราคา</span><strong>${formatDate(quotation.quote_date)}</strong></div>
        <div class="kv-row"><span>วันหมดอายุ</span><strong>${formatDate(quotation.valid_until)}</strong></div>
        <div class="kv-row"><span>ผู้เสนอราคา</span><strong>${escapeHTML(model.ownerName || "-")}</strong></div>
      </div>
    </div>

    ${renderQuotationDocumentManagerV1121(model, driveLog, settings)}

    <div class="card">
      <div class="card-header compact-v1120"><div><h3>${quotation.billing_type === "yearly" ? "ค่าบริการชำระรายปี" : "ค่าบริการชำระรายเดือน"}</h3></div></div>
      ${renderQuotationItemsTableV1121(recurringItems)}
    </div>

    <div class="card">
      <div class="card-header compact-v1120"><div><h3>ค่าบริการชำระครั้งเดียวจบ</h3></div></div>
      ${renderQuotationItemsTableV1121(oneTimeItems)}
    </div>

    <div class="card">
      <div class="card-header compact-v1120"><div><h3>สรุปยอด</h3></div></div>
      <div class="kv-list">
        <div class="kv-row"><span>มูลค่าก่อนภาษี</span><strong>${formatTHB(quotation.subtotal_amount)}</strong></div>
        <div class="kv-row"><span>ส่วนลด</span><strong>${formatTHB(quotation.discount_amount)}</strong></div>
        <div class="kv-row"><span>ฐานคำนวณภาษี</span><strong>${formatTHB(quotation.taxable_amount)}</strong></div>
        <div class="kv-row"><span>VAT ${Number(quotation.vat_rate || 0)}%</span><strong>${formatTHB(quotation.vat_amount)}</strong></div>
        <div class="kv-row"><span>หัก ณ ที่จ่าย ${Number(quotation.wht_rate || 0)}%</span><strong>-${formatTHB(quotation.wht_amount)}</strong></div>
        <div class="kv-row"><span>ส่วนต่างปัดเศษ</span><strong>${formatTHB(quotation.rounding_adjustment)}</strong></div>
        <div class="kv-row"><span>ยอดรวมสุทธิ</span><strong style="font-size:22px;">${formatTHB(quotation.grand_total_display)}</strong></div>
      </div>
    </div>

    <div class="card">
      <div class="card-header compact-v1120"><div><h3>หมายเหตุและเงื่อนไข</h3></div></div>
      <div class="kv-list">
        <div class="kv-row"><span>หมายเหตุ</span><strong style="white-space:pre-wrap;">${escapeHTML(quotation.note || "-")}</strong></div>
        <div class="kv-row"><span>เงื่อนไขการชำระเงิน</span><strong style="white-space:pre-wrap;">${escapeHTML(quotation.payment_terms || "-")}</strong></div>
      </div>
    </div>
  `;

  bindQuotationViewActionsV1121(quotation, model);

  if (quotation.status !== "draft") {
    renderDriveArchiveStatusV1102(model, driveLog, settings);
    const driveSaveButton = document.getElementById("saveDrivePdfButtonV1102");
    if (driveSaveButton) {
      const clone = driveSaveButton.cloneNode(true);
      driveSaveButton.replaceWith(clone);
      clone.addEventListener("click", async () => handleSaveQuotationPdfToDriveV1102(model));
    }
  }

  try {
    if (typeof injectJourneyIntoQuotationViewV1110 === "function") await injectJourneyIntoQuotationViewV1110(quotationId);
    if (typeof removeExtraQuotationJourneyCardsV1112 === "function") removeExtraQuotationJourneyCardsV1112();
  } catch (error) {
    console.warn("Journey injection skipped v1.12.1", error);
  }

  cleanupCopyV1120(elements.pageContent);
}

function bindQuotationViewActionsV1121(quotation, model) {
  document.getElementById("backToListButton")?.addEventListener("click", () => { location.hash = "#quotations"; });
  document.getElementById("editDraftButton")?.addEventListener("click", () => { location.hash = `#quotation-edit/${quotation.id}`; });
  document.getElementById("confirmQuotationButton")?.addEventListener("click", async () => confirmQuotation(quotation.id));
  document.getElementById("cancelQuotationButton")?.addEventListener("click", async () => cancelSingleQuotation(quotation.id));
  document.getElementById("markPaidButton")?.addEventListener("click", async () => markQuotationAsPaid(quotation.id));
  document.getElementById("printPreviewButton")?.addEventListener("click", () => { location.hash = `#quotation-print/${quotation.id}`; });
  document.getElementById("duplicateQuotationButton")?.addEventListener("click", async () => duplicateQuotation(quotation.id));
}

renderQuotationViewPage = renderQuotationViewPageV1121;
window.renderQuotationViewPage = renderQuotationViewPageV1121;

async function renderQuotationPrintPageV1121(quotationId) {
  if (!quotationId) {
    renderError("ไม่พบรหัสใบเสนอราคา");
    return;
  }

  setCompactPageHeaderV1120("ตัวอย่างเอกสาร / พิมพ์", "ตรวจสอบเอกสารก่อนพิมพ์หรือบันทึกเป็น PDF");
  renderLoading("กำลังโหลดเอกสาร...");

  const model = await loadQuotationPrintModelV1121(quotationId);
  if (model.quotation.status === "draft") {
    elements.pageContent.innerHTML = `
      <div class="card">
        <div class="card-header">
          <div><h3>ยังไม่สามารถพิมพ์เอกสารได้</h3></div>
          <button class="btn btn-ghost" onclick="location.hash='quotation-view/${model.quotation.id}'">กลับไปหน้ารายละเอียด</button>
        </div>
        <div class="empty-state compact">ต้องยืนยันใบเสนอราคาเพื่อสร้างเลขเอกสารก่อน</div>
      </div>
    `;
    return;
  }
  ...
  printButton?.addEventListener("click", () => {
    if (typeof printWithSuggestedFilenameV1101 === "function") printWithSuggestedFilenameV1101(model);
    else window.print();
  });
  backButton?.addEventListener("click", () => { location.hash = `#quotation-view/${model.quotation.id}`; });

  document.getElementById("driveArchiveStatusV1102")?.remove();
  document.getElementById("quotationPrintJourneyV1110")?.remove();
  requestAnimationFrame(() => applyPrintFitModeV15?.());
  window.setTimeout(() => applyPrintFitModeV15?.(), 350);
}

renderQuotationPrintPage = renderQuotationPrintPageV1121;
window.renderQuotationPrintPage = renderQuotationPrintPageV1121;

if (typeof initializeDriveArchiveToolbarV1102 === "function") {
  initializeDriveArchiveToolbarV1102 = async function initializeDriveArchiveToolbarV1121() {
    return null;
  };
}
...
function renderPrintV15ItemRow(item, index) {
  const lineSubtotal = item.line_subtotal ??
    (item.section_type === "recurring" ? item.unit_price : Number(item.quantity || 0) * Number(item.unit_price || 0));
  return `
    <tr>
      <td class="center">${index + 1}</td>
      <td>
      ...
      <td class="num">${number(item.quantity)}</td>
      <td class="center">${escapeHTML(item.unit || item.product_master_v1121?.default_unit || "-")}</td>
      <td class="num">${formatTHB(item.unit_price)}</td>
      <td class="num">${formatTHB(lineSubtotal)}</td>
    </tr>
  `;
...
async function loadQuotationEmailContextV1113(model, driveLog) {
  ...
  const [quotationResult, itemsResult, driveResult, emailSettings] = await Promise.all([
    ...
    supabaseClient
      .from("quotation_items")
      .select("section_type, product_id, product_name_snapshot")
      .eq("quotation_id", quotationId),
    driveLog?.file_url ? Promise.resolve({ data: driveLog, error: null }) : supabaseClient
      .from("quotation_drive_files")
      .select("*")
  ...
  const productsMap = await loadProductsMapV1121((itemsResult.data || []).map((item) => item.product_id));
  const items = enrichQuotationItemsWithProductMasterV1121(itemsResult.data || [], productsMap);
  const ownerProfile = await loadQuotationOwnerProfileV1113(quotationResult.data.owner_id);

  return {
    quotation: quotationResult.data,
    items,
    recurringItem: items.find((item) => item.section_type === "recurring") || items[0] || null,
    driveLog: driveResult.data || null,
    ownerProfile,
    emailSettings,
  };
...
function getQuotationServiceNameForEmailV1113(context) {
  const recurring = context?.recurringItem || (context?.items || []).find((item) => item.section_type === "recurring") || context?.items?.[0];
  return getItemDisplayNameV1121(recurring || {}) || "ซอฟต์แวร์ระบบ";
}
...
function renderStatusPieV1120(statusCounts) {
  ...
  const entries = [
    ["draft", "ร่าง", "#94a3b8"],
    ["confirmed", "ยืนยันแล้ว", "#0ea5e9"],
    ["sent", "ส่งแล้ว", "#7c3aed"],
    ["paid", "ชำระเงินแล้ว", "#16a34a"],
    ["expired", "หมดอายุ", "#f59e0b"],
  ...
  return `
    <div class="dashboard-chart-card-v1120">
      ...
      <div class="pie-wrap-v1120">
        ...
        <div class="pie-legend-v1120">
          ${entries.map(([key, label, color]) => `
            <span><i style="background:${color}"></i>${escapeHTML(label)} <b>${number(statusCounts[key] || 0)}</b></span>
          `).join("")}
        </div>
      </div>
    </div>
...
function renderFunnelV1120(statusCounts) {
  ...
  const total = Object.values(statusCounts).reduce((sum, value) => sum + Number(value || 0), 0) || 1;
  return `
    <div class="dashboard-chart-card-v1120 funnel-card-v1120">
      <div class="card-header compact-v1120"><div><h3>Funnel ใบเสนอราคา</h3></div></div>
      <div class="funnel-list-v1120">
        ${stages.map(([key, label]) => {
          const count = Number(statusCounts[key] || 0);
          const width = Math.max(0, Math.round((count / total) * 100));
          return `
            <div class="funnel-row-v1120">
              <div><strong>${escapeHTML(label)}</strong><span>${number(count)} / ${number(total)} ใบ</span></div>
              <div class="funnel-track-v1120"><i style="width:${width}%"></i></div>
              <b>${number(width)}%</b>
            </div>
          `;
        }).join("")}
...
function renderPaidTrendV1120(rows) {
  ...
  (rows || []).forEach((row) => {
    ...
    if (Number.isNaN(date.getTime())) return;
    const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
    if (!monthMap.has(key)) monthMap.set(key, { month: key, paidAmount: 0, paidCount: 0 });
    if (getStatusKeyV1120(row) === "paid") {
      const bucket = monthMap.get(key);
      bucket.paidAmount += Number(row.paid_amount_actual || 0);
      bucket.paidCount += 1;
    }
  ...
  const maxAmount = Math.max(...months.map((item) => item.paidAmount), 1);
  return `
    <div class="paid-bar-chart-v1121">
      ${months.map((item) => {
        const height = Math.max(4, Math.round((item.paidAmount / maxAmount) * 100));
        return `
          <div class="paid-bar-item-v1121">
            <strong>${formatTHB(item.paidAmount)}</strong>
            <div class="paid-bar-track-v1121"><i style="height:${height}%"></i></div>
            <span>${escapeHTML(item.month)}</span>
          </div>
        `;
      }).join("")}
...
async function loadDashboardItemsV1120(quotationIds) {
  ...
  try {
    const { data, error } = await supabaseClient
      .from("quotation_items")
      .select("quotation_id, section_type, product_id, product_name_snapshot, line_subtotal, quantity, unit_price")
      .in("quotation_id", ids);
    if (error) throw error;
    const productsMap = await loadProductsMapV1121((data || []).map((item) => item.product_id));
    return enrichQuotationItemsWithProductMasterV1121(data || [], productsMap);
  } catch (error) {
    console.warn("Top product data skipped v1.12.1", error);
    return [];
  }
}
...
function renderTopProductsV1120(items) {
  const map = new Map();
  (items || []).forEach((item) => {
    const name = getItemDisplayNameV1121(item);
    if (!name) return;
    if (!map.has(name)) map.set(name, { name, count: 0, amount: 0 });
    const row = map.get(name);
    row.count += 1;
...
const originalRenderQuotationsPageV1121 = typeof renderQuotationsPage === "function" ? renderQuotationsPage : null;
renderQuotationsPage = async function renderQuotationsPageV1121() {
  setCompactPageHeaderV1120(appState.profile.role === "sales" ? "ใบเสนอราคาของฉัน" : "ใบเสนอราคา", "ค้นหาและติดตามสถานะใบเสนอราคา");
  renderLoading("กำลังโหลดใบเสนอราคา...");

  const [quotationsResult, productsResult] = await Promise.all([
    supabaseClient.from("v_quotations_list").select("*").order("created_at", { ascending: false }),
    supabaseClient.from("products").select("id, code, name, is_active").order("created_at", { ascending: true }),
  ]);
  if (quotationsResult.error) throw quotationsResult.error;
  if (productsResult.error) throw productsResult.error;

  appState.productOptionsV1121 = productsResult.data || [];
  const rows = await enrichQuotationRowsWithPrimaryProductV1121(quotationsResult.data || []);
  renderQuotationList(rows);
};
window.renderQuotationsPage = renderQuotationsPage;

const originalRenderQuotationListV1121 = typeof renderQuotationList === "function" ? renderQuotationList : null;
if (originalRenderQuotationListV1121) {
  renderQuotationList = function renderQuotationListV1121(rows) {
    originalRenderQuotationListV1121(rows);
    injectProductFilterV1121();
  };
  window.renderQuotationList = renderQuotationList;
}

function injectProductFilterV1121() {
  if (document.getElementById("quotationProductFilter")) return;
  const billingField = document.getElementById("quotationBillingFilter")?.closest(".field");
  const target = billingField || document.querySelector(".filter-grid-v13");
  if (!target) return;
  const products = appState.productOptionsV1121 || [];
  const selected = appState.quotationFilters.productId || "";
  const field = document.createElement("div");
  field.className = "field";
  field.innerHTML = `
    <label for="quotationProductFilter">สินค้า/บริการ</label>
    <select id="quotationProductFilter">
      <option value="">ทั้งหมด</option>
      ${products.map((product) => `<option value="${escapeHTML(product.id)}" ${product.id === selected ? "selected" : ""}>${escapeHTML(getProductDisplayNameV1121(product))}</option>`).join("")}
    </select>
  `;
  if (billingField?.parentElement) billingField.insertAdjacentElement("afterend", field);
  else target.appendChild(field);

  document.getElementById("quotationProductFilter")?.addEventListener("change", () => {
    appState.quotationFilters.productId = document.getElementById("quotationProductFilter")?.value || "";
    if (appState.quotationPagination) appState.quotationPagination.page = 1;
    updateQuotationTableFromFilters();
  });
}
...
function updateQuotationTableFromFilters() {
  ...
  appState.quotationFilteredRows = (appState.quotationListRows || []).filter((row) => {
    const keyword = (filter.keyword || "").toLowerCase();
    const productText = getPrimaryProductLabelV1121(row);
    const text = `${row.quotation_no || ""} ${row.customer_name || ""} ${row.owner_name || ""} ${productText}`.toLowerCase();
    return (!keyword || text.includes(keyword)) &&
      (!filter.status || row.effective_status === filter.status || row.status === filter.status) &&
      (!filter.billingType || row.billing_type === filter.billingType) &&
      (!filter.ownerId || row.owner_id === filter.ownerId) &&
      (!filter.productId || row.product_id_v1121 === filter.productId) &&
      (!quoteRange.active || !quoteRange.valid || isDateWithinRange(row.quote_date, quoteRange.from, quoteRange.to)) &&
      (!validRange.active || !validRange.valid || isDateWithinRange(row.valid_until, validRange.from, validRange.to));
  });
...
function renderQuotationTable(rows) {
  ...
  return `
    <div class="table-wrap">
      <table class="data-table quotation-table-v1114 quotation-table-v1121">
        <thead>
          <tr>
            <th class="select-col"><input id="selectAllQuotations" type="checkbox" ${allSelected ? "checked" : ""} ${selectableRows.length ? "" : "disabled"} /></th>
            ${sortableTh("row_no", "ลำดับ")}
            ...
            ${sortableTh("customer_name", "ลูกค้า")}
            ${sortableTh("billing_type", "ประเภท")}
            ${sortableTh("product_name_v1121", "สินค้า/บริการ")}
            ${showSales ? sortableTh("owner_name", "ฝ่ายขาย") : ""}
            ${sortableTh("quote_date", "วันที่เสนอราคา")}
            ${sortableTh("valid_until", "วันหมดอายุ")}
            ${sortableTh("grand_total_display", "ยอดรวม")}
        ...
        <tbody>
          ${rows.map((row, index) => {
            ...
            return `
              <tr class="${selectable ? "" : "row-locked-v1114"}">
                <td class="select-col"><input type="checkbox" data-select-quotation="${row.id}" ${appState.selectedQuotationIds?.has(row.id) && selectable ? "checked" : ""} ${selectable ? "" : "disabled"} /></td>
                <td>${index + 1}</td>
                <td><strong>${escapeHTML(row.quotation_no || "ยังไม่ออกเลข")}</strong></td>
                <td>${escapeHTML(row.customer_name || "-")}</td>
                <td>${billingTypeLabel(row.billing_type)}</td>
                <td>${escapeHTML(getPrimaryProductLabelV1121(row))}</td>
                ${showSales ? `<td>${escapeHTML(row.owner_name || "-")}</td>` : ""}
                <td>${formatDate(row.quote_date)}</td>
                <td>${formatDate(row.valid_until)}</td>
                <td class="num-cell">${formatTHB(row.grand_total_display)}</td>
...
const originalGetSortValueV1121 = typeof getSortValue === "function" ? getSortValue : null;
if (originalGetSortValueV1121) {
  getSortValue = function getSortValueV1121(row, key) {
    if (key === "product_name_v1121") return getPrimaryProductLabelV1121(row).toLowerCase();
    return originalGetSortValueV1121(row, key);
  };
}

const originalDebugV1121 = window.FI_DEBUG;
window.FI_DEBUG = async function FI_DEBUG_V1121() {
  const result = typeof originalDebugV1121 === "function" ? await originalDebugV1121() : {};
  return {
    ...result,
    version: FI_V1121_VERSION,
    actionsMovedToQuotationView: true,
    printPageActionsLimitedToPrint: true,
    funnelUsesTotalAsDenominator: true,
    paidTrendBarChart: true,
    productMasterNameById: true,
    quotationProductFilter: true,
    sqlChanged: false,
    appsScriptChanged: false,
  };
};
